...
async def async_validate(hass: HomeAssistant) -> EnergyPreferencesValidation:
    ...
    for source in manager.data["energy_sources"]:
        ...
        validation_function = SOURCE_VALIDATION_FUNCTIONS.get(source["type"])
        if validation_function:
            validate_calls.extend(
                validation_function(hass, source, source_result, wanted_statistics_metadata)
            )
...
def validate_grid_source(hass, source, source_result, wanted_statistics_metadata):
    validate_calls = []
...
def validate_gas_source(hass, source, source_result, wanted_statistics_metadata):
    validate_calls = []
    return validate_calls


def validate_solar_source(hass, source, source_result, wanted_statistics_metadata):
    validate_calls = []
    return validate_calls
...
SOURCE_VALIDATION_FUNCTIONS = {
    "grid": validate_grid_source,
    "gas": validate_gas_source,
    "solar": validate_solar_source,
}
