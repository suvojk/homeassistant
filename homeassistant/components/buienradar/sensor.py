...
def _load_data(self, data):  
    ...
    if sensor_type.endswith(tuple(["_1d", "_2d", "_3d", "_4d", "_5d"])):
        return self._update_forecast_sensors(data, sensor_type)

    if sensor_type in [SYMBOL, *CONDITION_TYPES]:
        return self._update_condition_sensors(data, sensor_type)

    if sensor_type.startswith(PRECIPITATION_FORECAST):
        return self._update_precipitation_forecast_sensors(data, sensor_type)

    if sensor_type in [WINDSPEED, WINDGUST, VISIBILITY]:
        return self._update_wind_and_visibility_sensors(data, sensor_type)
